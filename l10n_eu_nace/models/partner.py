...
class PartnerNaceCategory(models.Model):
    """Let users search on code without a dot"""
    _inherit = 'res.partner.category'
    _name = 'res.partner.nace'

    parent_id = fields.Many2one('res.partner.nace', string='Parent Category', index=True, ondelete='cascade')
    child_ids = fields.One2many('res.partner.nace', 'parent_id', string='Child Tags')


    @api.model
    def name_search(self, name, args=None, operator='ilike', limit=100):
        """When no results are found, try again with an additional "."."""
        results = super(PartnerNaceCategory, self).name_search(
